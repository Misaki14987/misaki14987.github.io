...
try {
  ...
  await waitFor("document.documentElement.dataset.theme", "plain");
  await waitFor("localStorage.getItem('site-theme')", "plain");
  ...
  await evaluate("document.querySelector('.units-post').click()");
  ...
  if (after !== "plain" || storedAfter !== "plain") process.exitCode = 1;
} finally {
  socket?.close();
