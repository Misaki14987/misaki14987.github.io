...
等刚接通信号时的暂态过程结束以后，输出仍然应该是是相同频率的正弦信号，不过幅度和相位可能已经发生了变化：
...
这里有个问题：输入和输出都是正弦信号，方程中却还有输出的导数。如果直接展开三角函数当然也可以算，但是过程很麻烦，我们比较懒，所以我们希望找到一种表示方法，可以把求导变成普通的乘法。这个时候就用到复指数
...
H(jw)就是在输入频率为 w 时，输出和输入的复数幅值之比
...
到这里，最开始提出的两个问题已经有答案了：我们的到了相位和幅度是怎么变的。但是我们现在只写出了公式，还没有看出频率改变时结果究竟会怎么具体变化，所以我们来变一下频率看看
...
还有另一种画法。对于每个频率，$H(j\omega)$ 本身就是一个复数，所以也可以直接把它画在复平面上。随着 $\omega$ 不断变化，这个点会形成一条轨迹，这就是 **奈奎斯特图**

![同一频率响应的 Bode 图和 Nyquist 图](./frequency-response.svg)

*图中相同颜色的点来自同一个频率*

所以 Bode 图和奈奎斯特图不是两组不同的数据：
...
所以上一篇博客真正关心的是另一个问题：把外部输入撤掉以后，系统自身产生的扰动会衰减还是增长？这才是稳定性问题。只观察等幅正弦输入还不够，所以接下来要把 $j\omega$ 扩展成更一般的 $s$
...
在 $s$ 平面上取一条沿右半平面边界闭合的 **奈奎斯特围线**。它沿虚轴前进，再通过右半平面内的一个大半圆闭合，最后让半圆半径趋向无穷大。本文仍然约定这条围线沿逆时针方向前进
...
这就是在本文使用的环路增益定义下，奈奎斯特图需要观察临界点 $T=1$ 的原因
...
这两个条件同时满足就表示 $T(j\omega)=1$，也就是奈奎斯特轨迹刚好经过临界点 $1$。此时：
...
巴克豪森准则只检查有没有某一个频率刚好满足振荡条件，而 奈奎斯特判据会观察一条完整的闭合轨迹，再结合开环右半平面极点数 $P$，计算闭环右半平面极点数 $Z$
